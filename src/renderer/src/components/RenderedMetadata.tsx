...
import { filterAttributes, getAttributeConfig, Note, RepositoryConfig } from '@renderer/Model'
...
export function RenderedAttributes({
  ...
  if (!filteredAttributes || Object.keys(filteredAttributes).length === 0) return null
  return (
...
function RenderedMetadata({ note, showTags = true, showAttributes = true }: RenderedMetadataProps) {
  const { config } = useContext(ConfigContext)
  const repositoryConfig = config.repositories[note.repositorySlug]

  const filteredAttributes = filterAttributes(note.attributes, defaultOmitAttributes)
  ...
  return (
    <div className="RenderedMetadata">
      ...
      {showAttributes && filteredAttributes && (
        <RenderedAttributes attributes={filteredAttributes} repositoryConfig={repositoryConfig} />
      )}
    </div>
  )
}
