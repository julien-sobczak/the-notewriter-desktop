...
function KanbanRenderedColumn({ column, items, renderItem }: KanbanRenderedColumnProps) {
  ...
  const [selectedStatuses, setSelectedStatuses] = useState<Set<string>>(() => {
    ...
    return new Set(
      column.statuses.filter((status) => !hiddenStatuses.includes(status.toLowerCase()))
    )
  })
      ...
      <div className="KanbanFilter">
        <ul className="Filter">
          {column.statuses.map((status) => (
            ...
            >
              {determineLongAttributeLabel(defaultRepositoryConfig, 'status', status)}
            </li>
          ))}
        </ul>
      </div>
            ...
            <div key={`${column.title}-item-${index}`} className="KanbanItem">
              <header>
                <span className="KanbanItemStatus">
                  {determineShortAttributeLabel(defaultRepositoryConfig, 'status', item.status)}
                </span>
                &nbsp;
...
function KanbanBoard({ columns, items, renderItem }: KanbanBoardProps) {
  ...
  const filteredItemsByGroup = items.filter((item) => selectedGroups.has(item.group))
