...
import { FolderIcon as FolderCloseIcon, FolderOpenIcon, FileTextIcon } from '@phosphor-icons/react'
import TreeView, { ITreeViewOnNodeSelectProps, flattenTree } from 'react-accessible-treeview'
import { File, Note, RepositoryRefConfig } from '@renderer/Model'
...
function Browser({ file }: BrowserProps) {
  ...
  useEffect(() => {
    ...
    const loadFiles = async () => {
      const result: File[] = await window.api.listFiles(selectedRepository)
      // console.log(result);
  ...
  useEffect(() => {
    ...
    const listNotesInFile = async () => {
      const result: Note[] = await window.api.listNotesInFile(selectedRepository, selectedFile)
      setNotes(result)
    }
    listNotesInFile()
  }, [selectedFile])
  ...
  return (
    <div className="Browser">
      ...
      <div className="LeftPanel">
        <select value={selectedRepository} onChange={(e) => handleRepositoryChange(e.target.value)}>
