...
import { Bookmark, Note, NoteRef, QueryResult } from '@renderer/Model'
...
function Bookmarker({ bookmark }: BookmarkerProps) {
  ...
  const [selectedBookmark, setSelectedBookmark] = useState<Bookmark | null | undefined>(bookmark)
  ...
  useEffect(() => {
    ...
    const find = async () => {
      const note: Note = await window.api.find(noteRef)
          ...
          <ul>
            {notes.map((note: Note) => (
              ...
              >
                ...
                <br />
                <span className="BookmarkRelativePath">{note.relativePath}</span>
              </li>
            ))}
            {(bookmarks ?? []).map((savedBookmark: Bookmark) => (
              <li
                ...
                className={classNames({
                  selected: selectedBookmark && selectedBookmark.noteOID === savedBookmark.noteOID
                })}
              ...
              >
                ...
                <br />
                <span className="BookmarkRelativePath">{savedBookmark.noteRelativePath}</span>
              </li>
            ))}
          </ul>
        )}
