...
import { ConfigContext, selectedQueriesInspiration } from '@renderer/ConfigContext'
...
function Inspiration() {
  ...
  const [inspirationQueries, setInspirationQueries] = useState<QueryConfigWithContext[]>([])
  ...
  useEffect(() => {
    const loadInspirations = () => {
      const queries = selectedQueriesInspiration(config)
      setInspirationQueries(queries)
    }
  ...
  if (inspirationQueries.length === 0) {
  ...
  const loadNotes = (inspiration: QueryConfigWithContext) => {
    ...
    console.debug(`Searching for ${inspiration.title}...`)
